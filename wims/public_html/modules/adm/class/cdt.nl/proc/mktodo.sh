...
if [ "$w_cdt_supervise" = "yes" ]; then
 cat $w_wims_home/log/classes/$w_wims_class/cdt/index | awk -F"," -v dir=$w_wims_class -v jourb=$w_jour_begin -v joure=$w_jour_end '$1>=jourb && $1<=joure {print $0","dir;}' | sort | head -c 15k;
else 
