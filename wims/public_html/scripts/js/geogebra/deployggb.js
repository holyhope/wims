...
var GGBApplet = function() {
  ...
  applet.inject = function() {
    ...
    function continueInject() {
      ...
      if (type === 'screenshot') {
        ...
        if (parameters.hasOwnProperty('playButton') && parameters.playButton) {
          playButton = true
        } else if (parameters.hasOwnProperty('playButtonAutoDecide') && parameters.playButtonAutoDecide) {
          playButton = isMobileDevice()
        }
        if (playButton) {
  ...
  function getWidthHeight(appletElem, appletWidth) {
    ...
    if (appletElem) {
      ...
      var border = 0;
      if (rect.left > 0) {
        if (rect.left <= myWidth) {
          var borderLeft = rect.left;
          var borderRight = myWidth <= 640 ? 10 : 30;
          border = borderLeft + borderRight
        }
      }
      if (appletWidth > 0 && appletWidth + border < myWidth) {
        myWidth = appletWidth
      } else {
        myWidth -= border
      }
  ...
  function calcScale(isScreenshoGenerator, appletElem, appletWidth) {
    ...
    var windowSize = getWidthHeight(appletElem, appletWidth);
  ...
  var injectJavaApplet = function(appletElem, parameters) {
    if (views.CV) {
      ...
      script.innerHTML = '' + '       var _GIAC_caseval = \'nD\';' + '       function _ggbCallGiac(exp) {' + '           var ret = _GIAC_caseval(exp);' + '           return ret;' + '       }';
      appletElem.appendChild(script)
    }
  ...
  var injectHTML5Applet = function(appletElem, parameters, noPreview) {
    ...
    if (parameters.width !== undefined) {
      if (parseVersion(html5CodebaseVersion) <= 4.4) {
        ...
        if (valBoolean(parameters.showToolBar) || valBoolean(parameters.showMenuBar)) {
          ...
          if (parameters.customToolbar !== undefined) {
            var tbIcons = parameters.customToolbar.split('|');
            minWidth = 45 * tbIcons.length + (valBoolean(parameters.showMenuBar) ? 186 : 88) + 22
          } else {
            minWidth = valBoolean(parameters.showMenuBar) ? 745 : 560
          }
        }
    ...
    if (!noPreview && parameters.width !== undefined) {
      ...
      previewPositioner.style.width = scale * oriWidth + 'px';
      previewPositioner.style.height = scale * oriHeight + 'px';
      ...
      if (parseVersion(html5CodebaseVersion) >= 5) {
        ...
        if (scale && scale !== 1 && !isNaN(scale) && (!isFlexibleWorksheetEditor() || parameters.hasOwnProperty('scale'))) {
    ...
    window.onresize = function() {
      ...
      if (!isFlexibleWorksheetEditor() && !isNaN(scale) && scale !== 1) {
        ...
        scaleElem.style.transform = 'scale(' + scale + ',' + scale + ')';
        scaleElem.style.transformOrigin = '0% 0% 0px';
        scaleElem.style.width = parameters.width * scale + 'px';
        scaleElem.style.height = parameters.height * scale + 'px'
      }
      if (oldOnResize !== null) {
        oldOnResize()
      }
  ...
  var injectScreenshot = function(appletElem, parameters, showPlayButton) {
    ...
    appletElem.appendChild(previewPositioner);
    var scale = getScale(parameters, appletElem);
    if (scale !== 1 && !isNaN(scale) && !isFlexibleWorksheetEditor()) {
  ...
  var injectPlayButton = function(appletElem, parameters, noPreview, type) {
    ...
    var imgs = appletElem.getElementsByTagName('img');
    for (var i = 0; i < imgs.length; i++) {
      if (imgs[i].className === 'ggb_preview_play') {
  ...
  var createScreenShotDiv = function(oriWidth, oriHeight, borderColor, showPlayButton) {
    ...
    if (previewLoadingPath !== null || previewPlayPath !== null && showPlayButton) {
      ...
      if (previewPlayPath !== null && showPlayButton) {
        pWidth = 80;
        pHeight = 80;
        previewOverlay.setAttribute('src', previewPlayPath);
        previewOverlay.style.cursor = 'pointer';
        previewOverlay.className = 'ggb_preview_play'
      } else {
        pWidth = 360;
      ...
      previewOverlay.setAttribute('width', pWidth - 4);
      previewOverlay.setAttribute('height', pHeight - 4);
  ...
  var setDefaultHTML5CodebaseForVersion = function(version, offline) {
    ...
    if (!isFlexibleWorksheet() && hasWebSimple && !views.is3D && !views.AV && !views.SV && !views.CV && !views.EV2 && !views.CP && !views.PC && !views.DA && !views.FI && !views.PV && !valBoolean(parameters.showToolBar) && !valBoolean(parameters.showMenuBar) && !valBoolean(parameters.showAlgebraInput) && !valBoolean(parameters.enableRightClick)) {
      codebase += 'webSimple/'
    } else {
      var v = parseVersion(html5CodebaseVersion);
  ...
  var setDefaultJavaCodebaseForVersion = function(version) {
    ...
    if (parseVersion(javaCodebaseVersion) < 4) {
      javaCodebaseVersion = '4.0'
    }
    var protocol;
    if (window.location.protocol.substr(0, 4) === 'http') {
