...
/* default add the drag code : nearly always used ...*/
  add_drag_code(js_include_file,DRAG_CANVAS,canvas_root_id);
	...
	*/
	...
	case MULTIDRAW:
	/*
	 @ multidraw obj_type_1,obj_type_2...obj_type_10
	 @ for simple single object user drawings you could also use command <a href="#userdraw">'userdraw'</a>
	 @ implemented obj_types:<ul><li>point | points </li><li>circle | circles </li><li>line | lines </li><li>segment | segments </li><li>arrow | arrows <br />use command 'arrowhead int' for size (default value 8 pixels)</li><li>rect | rects </li><li>closedpoly<br /><b>only one</b> closedpolygon may be drawn.The number of 'corner points' is not preset (e.g. not limited,freestyle)<br />the polygone is closed when clicking on the first point again..(+/- 10px) </li><li>triangle | triangles<br />parallelogram | parallelograms <br />mutually exclusive...if parallelogram(s) are to be combined with triangles, use poly3 or polys3 for triangle drawings <br />(<em>no inputfields for command parallelogram... it may be used for vector "contructions"</em>)</li><li>poly[3-9] | polys[3-9] draw a 3-9 point polygone: poly3 is a triangle </li></ul>
