!set lang_exists=yes

!set name_0=in bewerking
!set name_1=open
!set name_2=gesloten
!set name_3=onzichtbaar
!set name_publish=publiceren
!set name_publish2=publiceren
!set name_hide=verbergen
!set name_hide2=verbergen
!set name_nominative=te naam gestelde [niet anoniem]
!set name_anonymous=anoniem
!set name_trace=anoniem
!set name_choice=Keuze
!set name_data=gegevens
!set name_global=globaal
!set name_detail=gedetailleerd
!set name_help=Help
!set name_yourchoice=Your choice:
!set title_data=Gegevens vragenlijsten voor spreadsheet
!set title_nominative=enquete
!set title_vote=Vragenlijst 
!set title_anonymous=stemming
!set name_choose= -- Kies --
!set wims_name_vote=Enquete of verkiezings home lijsten
!set name_result=Resultaat van de enquete&nbsp;
!set name_content=Inhoud
!set name_creation=Maken van een enquete of verkiezing.
!set name_edition=Bewerken van een enquete of verkiezing.
!set name_remaining=rest
!set name_untitled=Untitled

!distribute lines  Jouw klas heeft geen voorziening voor een stemming , verkiezing of enquete.\
 Neem hierover kontact op met je docent.\
 Er is geen actieve stemming/verkiezing in je klas.\
 Hier zijn de stemmingen, verkiezingen of enquetes van jouw klas.\
 Zonder Titel\
into name_novote,name_novotecheck,name_noactivevote,name_class_vote,\
name_cnt_votes

 !distribute line Dit is een anonieme stemming/enquete.\
   Deze enquete is $name_trace.\
   Deze enquest is $name_nominative.\
   Het is gesloten.\
   votes\
   U hebt reeds gestemd.\
   Uw voorkeur is reeds genoteerd; maar U kunt zich nog bedenken\
   Uw Keuze\
   Détails\
   Opmerkingen\
   Gebruik het forum voor eventueel commentaar.\
into name_thisvote_anomymous,name_thisvote_trace,name_thisvote_nominatif,\
   name_thisvote_closed,name_thisvote_answer,name_thisvote_already1,\
   name_thisvote_already2,name_give_your_choice,name_details,\
   name_comments,name_textarea

!set name_whovote=Liste des votants
!set name_csv1=Hier is het bestand dat geopend kan worden met een spreadsheet\ 
 programma naar keuze

!set name_csv2=!nosubst U wilt de gegevens downloaden 
uit de vragenlijst nummer $vote
!set name_format=formaat